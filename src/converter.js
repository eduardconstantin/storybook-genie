...
export async function componentConverter(component, model, template, apiType = "openai") {
  ...
  try {
    const client = getClient(apiType); 
    if (apiType === "openai") {
      ...
      const response = await client.runOllamaModel(model, component);
      return response;
    }
  } catch (error) {
    console.error(error);
